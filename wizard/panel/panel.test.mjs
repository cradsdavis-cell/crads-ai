...
test('pending-devices + broker-register are RETIRED (2026-09-01): no broker remains', async () => {
  ...
  // verb may come back.
  assert.equal(MEMBER_VERBS['pending-devices'], undefined, 'pending-devices stays gone');
  assert.equal(MEMBER_VERBS['broker-register'], undefined, 'broker-register stays gone');
...
test('no panel verb can enrol a device: the box decides, on the invite-token binding', () => {
  assert.equal(MEMBER_VERBS['approve-device'], undefined,
...
test('member verb table: no chat anywhere, no org verbs, no adminOnly', () => {
  assert.equal(MEMBER_VERBS.chat, undefined, 'chat was removed from the org edition too (2026-07-23)');
...
test('support role: the commons verbs are unknown (not merely refused), plain reads still answer', async () => {
  const s = await listen({ bridge: fakeBridge([ROCK]), role: 'support' });
  try {
    assert.equal((await post(s, { host: 'ic-rock', verb: 'commons-publish' })).status, 400);
...
test('ruling 10 (2026-08-10), completed by the collapse: no verb births a mineral for somebody else', () => {
  ...
  assert.equal(MEMBER_VERBS['stamp-member'], undefined, 'the direct-key stamp verb stays gone');
  assert.equal(MEMBER_VERBS['invite-member'], undefined, 'and the invite-stamp path went with the Members page');
  assert.equal(MEMBER_VERBS['stamp-member'], undefined);
...
test('devices verbs: validation + shapes (device roster phase 1)', () => {
  ...
  // the member's own box: no admin gate
  for (const v of ['devices-list', 'devices-add', 'devices-revoke', 'devices-rename']) {
    assert.ok(!MEMBER_VERBS[v].adminOnly, `${v} not adminOnly`);
  }
});

test('no served verb is adminOnly any more (the commons write verbs left 2026-09-09); reads run unprefixed', async () => {
  // commons-publish was the served table's last adminOnly verb. With the
  // Catalogue gone nothing a member reaches is admin-gated, and the box-side
  // AIOS_LOGIN guard is never prefixed onto a served command.
  assert.ok(!Object.values(MEMBER_VERBS).some((v) => v && v.adminOnly), 'the one table carries no adminOnly verb');
  const bridge = fakeBridge([ROCK]);
  const s = await listen({ bridge });
  try {
    await post(s, { host: 'ic-rock', verb: 'skills-list' }).then((r) => r.text());
    assert.ok(!bridge.ran[0].command.includes('AIOS_LOGIN'), 'non-admin verbs unprefixed');
  } finally { s.close(); }
});
...
test('invite-member is RETIRED (2026-09-01), and no served verb reaches stamp-pebble', () => {
  ...
  assert.equal(MEMBER_VERBS['invite-member'], undefined, 'the birth verb stays gone');
  for (const [name, spec] of Object.entries(MEMBER_VERBS)) {
...
test('device-activity is RETIRED (2026-09-01): the auto-approve feed died with the broker', () => {
  ...
  assert.equal(MEMBER_VERBS['device-activity'], undefined, 'the verb stays gone');
...
test('P4 join verbs are RETIRED (2026-09-01): joining is a bundle, not a queue', () => {
  ...
  for (const v of ['join-requests', 'join-approve', 'join-decline']) {
    assert.equal(MEMBER_VERBS[v], undefined, `${v} stays gone`);
    assert.equal(MEMBER_VERBS[v], undefined, `${v} never reaches the one table`);
  }
  // the bundle path that replaced the queue left too (community surfaces retired 2026-09-09)
  assert.equal(MEMBER_VERBS['community-join'], undefined, 'nothing joins anything any more');
});
...
test('invite-reissue is RETIRED (2026-09-01): no rock mints device links at all', () => {
  ...
  assert.equal(MEMBER_VERBS['invite-reissue'], undefined, 'the verb stays gone');
  ...
  assert.equal(MEMBER_VERBS['invite-pending-list'], undefined, 'and no invite queue to poll');
...
test('no member verb is adminOnly (the org table, where adminOnly meant something, is gone)', () => {
...
test('T3.1: the ownership choice is RETIRED with the birth verb', () => {
  assert.equal(MEMBER_VERBS['invite-member'], undefined, 'no verb carries an --owner flag to stamp-pebble');
});
...
test('console-answer is RETIRED (2026-09-01): no directory holds requests to answer', () => {
  ...
  assert.equal(MEMBER_VERBS['console-answer'], undefined, 'the verb stays gone');
...
test('member-console-state: reads only the box\'s own truth, emits CONSOLE_STATE', () => {
  ...
  for (const f of ['anchored', 'name:nm', 'backup', 'custody']) {
    assert.match(c, new RegExp('[{,]' + f.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '[,}]'),
      `${f} rides the CONSOLE_STATE payload`);
  }
  // ties and the in-flight promotion left the payload with the Map and the
  // tie machinery (2026-09-09): nothing draws or resumes them any more
  assert.ok(!/ties\.json|promotion-pending/.test(c), 'no tie or promotion file is read');
...
test('console-request and console-withdraw are RETIRED (2026-09-01): no request fabric', () => {
  ...
  assert.equal(MEMBER_VERBS['console-request'], undefined, 'the create verb stays gone');
  assert.equal(MEMBER_VERBS['console-withdraw'], undefined, 'and the withdraw verb with it');
...
test('rock-answer and rock-tie-end are RETIRED: no directory holds ties to answer or end', () => {
  ...
  assert.equal(MEMBER_VERBS['rock-answer'], undefined, 'the tie answer verb stays gone');
  assert.equal(MEMBER_VERBS['rock-tie-end'], undefined, 'and the tie ender with it');
...
test('the tie ask routes are gone, and nothing dials a directory through them', async () => {
  ...
  try {
    ...
    try {
      ...
      // the community verbs that replaced these routes are gone too (2026-09-09)
      for (const v of ['community-join', 'community-leave', 'community-list', 'community-check', 'community-seen', 'community-share']) {
        assert.equal(MEMBER_VERBS[v], undefined, `${v} stays out of the one table`);
      }
    } finally { s.close(); }
  } finally { globalThis.fetch = realFetch; }
});
...
test('the tie slug plumbing is RETIRED, and so is leaving (nothing to leave since 2026-09-09)', () => {
  assert.equal(MEMBER_VERBS['rock-tie-end'], undefined, 'the tie ender stays gone');
  assert.equal(MEMBER_VERBS['community-leave'], undefined, 'no leave verb: there are no communities');
});
